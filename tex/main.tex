...
The Newton-Raphson method, often simply called Newton's method, has its origins in the late 17th century. Isaac Newton first described a rudimentary version of the method in 1669, strictly applying it to polynomial equations by substituting successive approximations (Ypma, 1995). In 1690, Joseph Raphson published a simplified, generalized version of Newton's process that abstracted the method into the iterative sequence we recognize today, though still primarily for polynomials (Ypma, 1995). It was not until 1740 that Thomas Simpson applied formalized calculus and derivatives to extend the method to non-linear and transcendental functions, finalizing the modern formula \parencite{ypma1995historical}. Mathematically, the method relies on approximating a function near a point pn by its tangent line and using the x-intercept of the tangent as the next approximation. This process uses local linearization based on the first-order Taylor series expansion (Burden et al., 2015). If pn is close to the true root p, setting the first-order Taylor expansion equal to zero yields the Newton iteration formula for the next approximation:
